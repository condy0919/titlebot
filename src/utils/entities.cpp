...
static std::string WriteUTF8(unsigned int ch) {
    char buf[4] = "";
    if (ch >= 0x800) {
        buf[0] = 0xE0 | ((ch >> 12) & 0x0F);
        buf[1] = 0x80 | ((ch >> 6) & 0x3f);
        buf[2] = 0x80 | (ch & 0x3f);
        buf[3] = '\0';
    } else if (ch >= 0x80) {
        buf[0] = 0xc0 | ((ch >> 6) & 0x1f);
        buf[1] = 0x80 | (ch & 0x3f);
        buf[2] = '\0';
    } else {
        buf[0] = ch;
...
std::string UnquoteHTML(std::string in) {
    enum {
        NoMatch,
        MatchBegin,
        MatchName,
    ...
    for (const unsigned char ThisCh : in) {
        ProcessedChar = false;
        GotCharCode = false;
        switch (MatchState) {
        case NoMatch:
        ...
        case MatchName:
            if ((ThisCh >= 'a' and ThisCh <= 'z') ||
                ...
                if (EntityNames.empty()) {
                    ...
                    for (;;) {
                        if (ThisEntry->name_ == NULL)
                            break;
                        EntityNames.insert(EntityNamePair(
                            std::string(ThisEntry->name_), ThisEntry->value_));
                        ++ThisEntry;
                    } /*for*/
                }     /*if*/
...
std::string Unescape(std::string in) {
    return detail::UnquoteHTML(std::move(in));
}
}
